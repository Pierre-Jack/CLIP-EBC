...
#not currently used      
class CrowdRaw(Dataset): 
    # Return raw data (e.g., file path, or an encoded JPEG buffer)
    # DALI will handle the decoding and augmentations later
    # num crops=1 for now
    # density map generation moved inside dali pipeline
    def __init__(
        self,
        dataset: str,
        split: str,
    ...
    def __getitem__(self, idx: int): #returns raw byte buffers (the encoded JPEG)
        ...
        # Read the raw JPEG bytes into a numpy array (uint8)
        # This is very fast on CPU and lets DALI's NVDEC take it from here
        with open(image_path, 'rb') as f:
            raw_img = np.frombuffer(f.read(), dtype=np.uint8)

        # Load the label npy (usually small)
        label = np.load(label_path).astype(np.float32)

        if self.return_filename:
            return raw_img, label, self.image_names[idx]
        return raw_img, label

# class CrowdDali(Dataset): 
#     # Return raw data (e.g., file path, or an encoded JPEG buffer)
#     # DALI will handle the decoding and augmentations later
#     # num crops=1 for now
#     # density map generation moved inside dali pipeline
...
def generate_impulse_map(label, height, width):
    density = np.zeros((height, width), dtype=np.float32)

    if len(label) > 0:
        label = label.astype(np.int64)
        label[:,0] = np.clip(label[:,0], 0, width - 1)
        label[:,1] = np.clip(label[:,1], 0, height - 1)
        density[label[:,1], label[:,0]] = 1.0
...
class CrowdDali(Dataset):
    ...
    def __init__(
        ...
        assert dataset.lower() in available_datasets
    ...
    def __getitem__(self, idx):
        ...
        # -------------------------------
        # Load image (H x W x C) as NumPy array
        # -------------------------------
        image = cv2.imread(image_path)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        # -------------------------------
        # Load label as NumPy array (N x 2)
        # -------------------------------
        label = np.load(label_path)  # shape (N, 2)

        # -------------------------------
        # Handle empty labels
        # -------------------------------
        if label.size == 0 or label.shape[1] == 0:
            label = np.empty((0, 2), dtype=np.float32)
        ...
        if label.numel() == 0:
            xs = torch.empty((0,), dtype=torch.int32)
            ys = torch.empty((0,), dtype=torch.int32)
        else:
            xs = label[:, 0].to(torch.int32)
            ys = label[:, 1].to(torch.int32)

        # -------------------------------
        # Impulse map (for DALI external source)
        # -------------------------------
        impulses = np.zeros((self.input_size, self.input_size), dtype=np.float32)

        # -------------------------------
        # Return
        # -------------------------------
        return image, label, impulses
